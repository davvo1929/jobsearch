...
public class PipelineService {
    ...
    private final ClaudeDraftGenerator  draftGenerator;
    private final ExecutorService       executor = Executors.newFixedThreadPool(6);

    public PipelineService(ClaudeJobSearchAgent jobSearchAgent, ClaudeDraftGenerator draftGenerator) {
    ...
    private Draft buildDraft(String resumeText, JobListing job, Consumer<Map<String, Object>> onEvent) {
        log.info("[{}] Writing cover letter for {} <{}>…", job.company(), job.recruiterName(), job.recruiterEmail());
        onEvent.accept(Map.of("type", "processing", "message", "Writing cover letter for " + job.company() + "…"));

        ClaudeDraftGenerator.GeneratedContent content = draftGenerator.generate(resumeText, job);
